...
Enifn, on va utiliser la fonction `observeEvent()` pour que le graphique soit modifiée selon le niveau d'analyse sur lequel on se situe, il n'est pas nécessaire ici de mettre une condition `if ... else ...` car le graphique est construit de la même façon et que nous utilisons une base de données réactive qui filtre selon la région quand l'utilisateur a sélectionné le niveau d'analyse régional.   
L'image suivante vous donne un aperçu de ce qu'on devrait avoir : 
