...
Des exemples d'application Shiny sont données sur la page officielle dans la <a href="https://shiny.rstudio.com/gallery/" target="_blank">gallerie</a> ; ou on peut aussi en lancer une directement dans R, avec par exemple le code suivant :
```{r eval=FALSE}
...
# principales à noter que comme elles ont le même nom, souvent on ne 
...
À savoir enfin, on peut également remplacer la fonction `sidebarLayout()` par une fonction `navbarPage()` pour permettre plusieurs onglets de navigation sur la même page web (que l'on créera avec la fonction `tabPanel()` à l'intérieur de laquelle on retrouvera les fonctions `sidebarLayout()` et `mainPanel()`), on y reviendra au cours du semestre...   
...
En réalité, il y a bien d'autres fonctions de réactivité dans Shiny (`eventReactive()`, `observeEvent()`, `reactiveValues()`, `isolate()`, `req()`), mais qu'on ne verra pas nécessairement ensemble cette année.  
